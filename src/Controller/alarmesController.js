
const {BigQuery} = require('@google-cloud/bigquery')

class alarmesController {

async alarmesCelulose(request,response){
    const bigquery = new BigQuery({
        projectId:'sz-00022-ws',
        credentials:{
            client_email:'sa-sami@sz-00022-ws.iam.gserviceaccount.com',
            private_key: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
        }
    });

    const {area} = request.body || ''
    const {datainicio} = request.body
    const {datafim} = request.body

    const dataInicioFormat = `${datainicio} 00:00:00.000`
    const dataFimFormat = `${datafim} 23:59:59.999`

    const {pagination} = request.body
    const limit = 1000
    const offset = pagination ===1? 0 : pagination*20
    const data = await bigquery.query(`SELECT alci_cd_identificador,alci_ds_tag,alci_tx_usuario_2,alci_ds_tipo_alarme_1,alci_tx_usuario_1,alci_dt_alarme,alci_dt_final,alci_ds_area FROM ${"`"}sz-00022-ws.ALARMES_E_EVENTOS.ALARMES_CELULOSE${"`"} WHERE alci_ds_origem = "Process(MB300)" AND LOWER(alci_ds_area) LIKE LOWER("%${area}%") AND (alci_dt_alarme>="${dataInicioFormat}" AND alci_dt_alarme<="${dataFimFormat}") ORDER BY alci_cd_identificador DESC LIMIT ${limit} OFFSET ${offset}`);
   
    
    return response.status(200).json(data[0])

}

async alarmesUtilidades(request,response){
    const bigquery = new BigQuery({
        projectId:'sz-00022-ws',
        credentials:{
            client_email:'sa-sami@sz-00022-ws.iam.gserviceaccount.com',
            private_key: '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n'
        }
    });

    const {area} = request.body || ''
    const {datainicio} = request.body
    const {datafim} = request.body

    const dataInicioFormat = `${datainicio} 00:00:00.000`
    const dataFimFormat = `${datafim} 23:59:59.999`

    const {pagination} = request.body
    const limit = 1000
    const offset = pagination ===1? 0 : pagination*20
    const data = await bigquery.query(`SELECT alci_cd_identificador,alci_ds_tag,alci_tx_usuario_2,alci_ds_tipo_alarme_1,alci_tx_usuario_1,alci_dt_alarme,alci_dt_final,alci_ds_area FROM ${"`"}sz-00022-ws.ALARMES_E_EVENTOS.ALARMES_UTILIDADES${"`"} WHERE alci_ds_origem = "Process(MB300)" AND LOWER(alci_ds_area) LIKE LOWER("%${area}%") AND (alci_dt_alarme>="${dataInicioFormat}" AND alci_dt_alarme<="${dataFimFormat}") ORDER BY alci_cd_identificador DESC LIMIT ${limit} OFFSET ${offset}`);
   
    
    return response.status(200).json(data[0])

}

}

module.exports = alarmesController
